...
if ($id_cabang == '0') {
    $kehadiran = mysqli_query($conn, "SELECT tbl_geolocation.ip, tbl_anggota.username, tbl_anggota.nama, tbl_anggota.alamat, tbl_absensi.status, tbl_absensi.tgl, tbl_absensi.jam, tbl_absensi.ttd FROM tbl_anggota INNER JOIN tbl_absensi ON tbl_anggota.username = tbl_absensi.username INNER JOIN tbl_geolocation ON tbl_anggota.username = tbl_geolocation.anggota_id where tbl_absensi.id_rat = '$status' ORDER BY RAND()");
} else {
